...
adjustBeta <- function(
             ...
             # Getting indices
             to_keep <- !is.na(methylation) & !is.na(purity)
             
             # Filtering data. Keep original betas for the output
             original_methylation <- methylation
             methylation <- methylation[to_keep]
             purity <- purity[to_keep]
             
             # Filtering data. Keep original sample names for the output
             original_names <- snames
             snames <- snames[to_keep]
             
             # Indices of NA values
             indices_of_NAs <- which(!to_keep)
             ...
             output_list$y.orig <- round(original_methylation,3)
             
             # Adding back NA values (if any)
             if (length(indices_of_NAs) > 0) {
               
               # Create full-length vectors with NA
               full_y_tum <- rep(NA, length(original_names))
               full_y_norm <- rep(NA, length(original_names))
               
               # Fill non-NA positions with computed values
               full_y_tum[to_keep] <- output_list$y.tum
               full_y_norm[to_keep] <- output_list$y.norm
               
               # Assign back to output list
               output_list$y.tum <- setNames(full_y_tum, original_names)
               output_list$y.norm <- setNames(full_y_norm, original_names)
             }
